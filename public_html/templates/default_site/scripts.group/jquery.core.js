...
jQuery(function(){
	...
	if ($('.datepicker').exists()) {
		...
		$( "#freeform_check-in" ).datepicker({
			dateFormat: 'd M yy',
			changeMonth: true,
			minDate: 0,	
			onSelect: function( selectedDate ) {
				$( "#freeform_check-out" ).datepicker( "option", "minDate", selectedDate );
			}
		});
		$( "#freeform_check-out" ).datepicker({
			dateFormat: 'd M yy',
			changeMonth: true,
			minDate: 0,
			onSelect: function( selectedDate ) {
				$( "#freeform_check-in" ).datepicker( "option", "maxDate", selectedDate );
			}
		});
	}
	...
	if ($('#thumbnails').exists()) {
		$('#thumbnails li a').click(function(){
			...
			$('.main-photo').find('.main-img').attr({'src': $(this).attr('href')});
			var imgTitle = $(this).attr('title');
			if(imgTitle != '') {
				$('.description').html(imgTitle );
			} else {
				$('.description').hide();
			}
			return false;
...
function initCarousel() {
	jQuery('div.gallery-section').scrollGallery({
		mask: 'div.gmask',
		slider: '>ul',
		slides: '>li',
