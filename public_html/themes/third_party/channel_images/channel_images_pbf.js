...
$(document).ready(function() {
	...
	ChannelImages.CFields.closest('form').bind('submit', function(){
		ChannelImages.CFields.find('form').find(':input').trigger('blur');
	});
...
ChannelImages.SyncOrderNumbers = function(FIELD_ID){
	...
	$('#ChannelImages_'+FIELD_ID).find('tbody.AssignedImages').find('tr.Image').each(function(Fileindex, Elem){
		...
		$(FILETD).find('input, textarea, select').each(function(){
...
ChannelImages.ActivateEditable = function(TargetTR, FIELD_ID){

	var jEvent = ChannelImages.Fields['Field_'+FIELD_ID].settings.jeditable_event;
	var placeholder = ChannelImages.LANG[jEvent+'2edit'];

	TargetTR.find('td[data-field=title]').editable(ChannelImages.EditFileDetails, {type:'text', placeholder: placeholder, onedit:ChannelImages.ActivateLiveUrlTitle, event: jEvent, onblur: 'submit'});
	TargetTR.find('td[data-field=url_title]').editable(ChannelImages.EditFileDetails, {type:'text', placeholder: placeholder, event: jEvent, onblur: 'submit'});
	TargetTR.find('td[data-field=description]').editable(ChannelImages.EditFileDetails, {type:'textarea', placeholder: placeholder, rows:2, event: jEvent, onblur:'submit'});
	TargetTR.find('td[data-field=category]').editable(ChannelImages.EditFileDetails, {type:'select', data: ChannelImages.Fields['Field_'+FIELD_ID].categories, placeholder: placeholder, event: jEvent, onblur:'submit'});
	TargetTR.find('td[data-field]').editable(ChannelImages.EditFileDetails, {type:'text', placeholder: placeholder, event: jEvent, onblur:'submit'});
};
...
ChannelImages.StoredImagesLoadEntryImages = function(Event){
	...
	$.get(ChannelImages.AJAX_URL, Params, function(rData){
		...
		Parent.find('.SearchImages .entryimages .images a').colorbox();
...
ChannelImages.RefreshImages = function(draft){
	...
	ChannelImages.CFields.each(function(i, e){
		...
		$.post(ChannelImages.AJAX_URL, Params, function(rData){
			...
			if (Called == false) ChannelImages.InitFields();
			Called = true;
