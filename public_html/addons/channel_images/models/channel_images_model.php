...
class Channel_images_model
{
	...
	public function parse_single_image_row($count, $image, $tagdata)
	{
		...
		$vars[$this->prefix.'title'] = $this->encode_html ? htmlentities($image->title, ENT_QUOTES) : $image->title;
		$vars[$this->prefix.'url_title'] = $image->url_title;
		$vars[$this->prefix.'description'] = $this->encode_html ? htmlentities($image->description, ENT_QUOTES) : $image->description;
		...
		$vars[$this->prefix.'field:1'] = $this->encode_html ? htmlentities($image->cifield_1, ENT_QUOTES) : $image->cifield_1;
		$vars[$this->prefix.'field:2'] = $this->encode_html ? htmlentities($image->cifield_2, ENT_QUOTES) : $image->cifield_2;
		$vars[$this->prefix.'field:3'] = $this->encode_html ? htmlentities($image->cifield_3, ENT_QUOTES) : $image->cifield_3;
		$vars[$this->prefix.'field:4'] = $this->encode_html ? htmlentities($image->cifield_4, ENT_QUOTES) : $image->cifield_4;
		$vars[$this->prefix.'field:5'] = $this->encode_html ? htmlentities($image->cifield_5, ENT_QUOTES) : $image->cifield_5;
	...
	public function get_channel_id($channels)
	{
		...
		if (strpos($channels, '|') !== FALSE)
		{
			...
			{
				$query = $this->EE->db->query("SELECT channel_id, channel_name FROM exp_channels WHERE channel_name IN ({$fixed}) ");
				if ($query->num_rows() == 0) return FALSE;
