...
class Freeform extends Module_builder_freeform
{
	...
	public function composer ($edit = FALSE)
	{
		...
		*/
				...
				foreach ($column as $field)
				{
					...
					else if ($field['type'] == 'nonfield_dynamic_recipients')
					{
						...
						if ($dynrec_data)
						{
							...
							{
								$fields['composer:field_output'] = '<ul class="dynamic_recipients">';

								foreach ($dynrec_output_array as $key => $value)
								{
									$fields['composer:field_output'] .= '<li><label>';
									$fields['composer:field_output'] .= form_checkbox(array(
										'name'	=> 'recipient_email[]',
										'value'	=> $key
									));
									$fields['composer:field_output'] .= '&nbsp;' . $value;
									$fields['composer:field_output'] .= '</label></li>';
								}
								$fields['composer:field_output'] .= '</ul>';
							}
							else
							{
								...
								$fields['composer:field_output'] = form_dropdown(
									'recipient_email',
									$dynrec_output_array
								);
							}
					...
					{
						$fields['composer:field_output'] = form_submit('submit', $field['html']);
					}
		...
		$tagdata = str_replace(
			array(
				'{composer:page}',
				'{' . T_SLASH . 'composer:page}'
			),
			array(
				'{composer:page}{composer:multi_page_start}',
				'{composer:multi_page_end}{' . T_SLASH . 'composer:page}'
			),
			$tagdata
		);
	...
	public function form ( $edit = FALSE, $preview = FALSE, $preview_fields = FALSE)
	{
		...
		if ( $multipage )
		{
			...
			if ($current_page == 0 AND $this->params['multipage_page_names'] != '')
			{
				...
				if ( ! empty($mp_page_array))
				{
					...
					{
						$page_names = explode('|', $this->params['multipage_page_names']);

						foreach ($page_names as $key => $value)
						{
							$page_names[$key] = preg_quote(trim($value), "/");
						}
			...
			//END page by name if ($current_page == 0)

			// -------------------------------------
			//	send numbers directly?
			// -------------------------------------
			...
			if ($current_page == 0)
			{
				$pm = preg_quote($this->params['page_marker'], '/');
			...
			else
			{
				...
				foreach ($page_matches[0] as $key => $value)
				{
					...
					//not current page. ignore and remove
					else
					{
						//remove
		...
		if ($edit)
		{
			$variables['freeform:edit_data:author']		= isset($edit_data['author']) ? $edit_data['author'] : '';
			$variables['freeform:edit_data:author_id']	= isset($edit_data['author_id']) ? $edit_data['author_id'] : '';
			$variables['freeform:edit_data:complete']	= isset($edit_data['complete']) ? $edit_data['complete'] : '';
			$variables['freeform:edit_data:edit_date']	= isset($edit_data['edit_date']) ? $edit_data['edit_date'] : '';
			$variables['freeform:edit_data:entry_date']	= isset($edit_data['entry_date']) ? $edit_data['entry_date'] : '';
			$variables['freeform:edit_data:entry_id']	= isset($edit_data['entry_id']) ? $edit_data['entry_id'] : '';
			$variables['freeform:edit_data:site_id']	= isset($edit_data['site_id']) ? $edit_data['site_id'] : '';
			$variables['freeform:edit_data:status']		= isset($edit_data['status']) ? $edit_data['status'] : '';
		}
	...
	public function save_form ($validate_only = FALSE)
	{
		...
		if ($multipage)
		{
			$entry_id = ee()->freeform_forms->store_multipage_entry(
				...
				$last_page,  //if true will finish up for us
				array(
